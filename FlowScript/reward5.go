	...
	for _, blockEvent := range blockEvents {
		for _, event := range blockEvent.Events {
			if all {
				...
				nodeID, delegatorID, amount, err := extractNodeIDAndAmount(event.Value.String())
				if err != nil {
				...
				if nodeID == TWM_NODE_ID {
					fmt.Println("\nFound our node...block ID =", blockEvent.BlockID.String(), " event transaction ID =", event.TransactionID, ", blockEnd =", blockEnd, ", amount =", amount)
					// print time stamp
					PrintTimeStamp(ctx, accessNodeClient, blockEvent)
					if delegatorID > 0 {
						fmt.Println("\tDelegatorID =", delegatorID)
					}
					fmt.Println("\tEvent type: " + event.Type)
					fmt.Println("\tEvent: " + event.Value.String())
					fmt.Println("\tEvent payload: " + string(event.Payload))
